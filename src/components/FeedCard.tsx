...
function FeedCard({
  ...
  return (
    <div className="flex gap-4 py-5 border-b border-gray-100/80 hover:bg-gray-50/50 transition cursor-pointer group px-2 -mx-2 rounded-lg">
      ...
      <div className="flex-1 w-full flex flex-col justify-center">
        <div className="flex items-center gap-2 mb-1">
          <div className={`w-[18px] h-[18px] rounded flex items-center justify-center text-[10px] font-bold text-white shadow-sm ${sourceColorClass}`}>
        ...
        <a href={link} target="_blank" rel="noopener noreferrer" className="inline-block" onClick={(e) => e.stopPropagation()}>
          <h3 className="font-bold text-[16px] text-gray-900 tracking-tight leading-snug mb-1 group-hover:text-blue-600 transition-colors duration-200">
            {title}
          </h3>
        </a>

        <p className="text-[13px] text-gray-500 leading-relaxed max-w-4xl pr-8 mt-1">
          {description}
        </p>
        ...
        <div className="flex items-center mt-3">
          ...
          <button
            ...
            className={`ml-auto text-[12px] font-medium transition-opacity ${
              isSaved ? "text-blue-600 opacity-100" : "text-gray-400 hover:text-blue-600 opacity-0 group-hover:opacity-100"
            }`}
          >
            {isSaved ? "Saved" : "Save"}
          </button>
